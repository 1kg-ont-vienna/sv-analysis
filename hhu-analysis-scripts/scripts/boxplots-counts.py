...
if args.vcfs:
    ...
    for vcf, name in zip(vcfs, names):
        ...
        writer = open(args.output+'/boxplot-counts-%s.tsv'%(name), 'w')
        ...
        for variant in vcf:
            ...
            for index, g in enumerate(variant.genotypes):
                ...
                if g[0]+g[1] == 1:
                    # HET SV    
                    data[sample][0][sv_type_to_index[sv_type]] += 1
                elif g[0]+g[1] == 2:
                    # HOM SV
                    data[sample][1][sv_type_to_index[sv_type]] += 1
        vcf.close()
    ...
    for sample, value in data_by_name.items():
        ...
        for c in ['all', cov]:
            for data_dict, stored_data in zip([het_data_by_name, hom_data_by_name], [value[0], value[1]]):
                data_dict[c][spop]['All'].append(sum(stored_data))
                data_dict[c][spop]['COMPLEX'].append(stored_data[0])
                data_dict[c][spop]['DEL'].append(stored_data[1])
                data_dict[c][spop]['INS'].append(stored_data[2])
...
for vtype in ['All', 'COMPLEX', 'DEL', 'INS']:
    for cov in covs:
        ...
        plt.savefig(args.output+'/boxplot-counts-HOM-%s-%s.svg'%(cov, vtype))
        plt.close()
        ...
        plt.tight_layout()
        plt.legend(handles, labels)
        plt.savefig(args.output+'/boxplot-counts-HET-%s-%s.svg'%(cov, vtype))
        plt.close()