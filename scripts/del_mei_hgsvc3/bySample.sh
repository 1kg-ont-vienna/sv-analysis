...
do
    ...
    do
	bcftools view -i '(STRLEN(REF)>=(STRLEN(ALT)+50))' -s ${SAMPLE} --min-ac 1 ont.vcf.gz | bcftools query -f "%CHROM\t%POS\t%END\t%ID\t%INFO/DTYPE_N\t%INFO/FAM_N\n" - | awk '{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6;}' | egrep "partnered|solo" | grep -w "${TAG}" | sort -k1,1V -k2,2n | uniq > ont.${SAMPLE}.${TAG}.bed
	zcat variants_T2T-CHM13_sv_insdel_HGSVC2024v1.0.tsv.gz | grep -v "^chrX" | grep -v "^chrY" | cut -f 1-6,9,16,17 | grep "${SAMPLE}" | awk 'BEGIN {FS="\t"; IFS="\t";} (($5=="DEL") && ($6>=50)) {print $2"\t"($3-'${WIN}')"\t"($4+'${WIN}')"\t"$1;}' | sort -k1,1V -k2,2n | uniq > hgsvc.${SAMPLE}.${TAG}.bed
	...
	SHAREDHGSVC=`bedtools intersect -a hgsvc.${SAMPLE}.${TAG}.bed -b ont.${SAMPLE}.${TAG}.bed -wao | awk '$5!="."' | cut -f 1-3 | sort | uniq | wc -l`
	UNIQHGSVC=`bedtools intersect -a hgsvc.${SAMPLE}.${TAG}.bed -b ont.${SAMPLE}.${TAG}.bed -wao | awk '$5=="."' | cut -f 1-3 | sort | uniq | wc -l`
	echo ${SAMPLE} ${TAG}
